...
$model = TophatterCategoryMap::find()->where(['merchant_id'=>$merchant_id])->andWhere(['!=','product_type',''])->all();
$data = TophatterCategory::find()->select('id,category_id,title,parent_id,level')->all();
$category_tree = array();
...
<div class="category-map-step category-map-index content-section">
    ...
    <form id="category_map" method="post" action="<?php echo \yii\helpers\Url::toRoute(['categorymap/save']) ?>">
		...
		<div style="display: none;border-radius: 4px;margin-bottom: 10px;padding: 10px;" class="help-block help-block-error top_error alert-danger">Please map atleast one product type with Tophatter category to list products</div>
		<div class="responsive-table-wrap">
			<table id="map_producttype" class="table table-striped table-bordered">
				<tr>
					<th>Id</th>
					<th>Product Category(bigcommerce)</th>
					<th style="width:25%;">Tophatter Tax Code <a href="<?= Data::getUrl('tophattertaxcodes/index');?>" target="_blank">(click here to get taxcode)</a></th>
					<th class="center" colspan="3"><label class="label-text-center">Tophatter Category Name</label></th>
				</tr>
				<?php 
				$i=0;
...
<script type="text/javascript">
var url = '<?= Data::getUrl("tophatter-install/save-category-map") ?>';
	...
	$('.next').on('click', function(event){	
		...
		$('.cat_root .root').each(function(){
			if($(this).val() == "") {
				...
				$('.top_error').html("Please map atleast one product type with Tophatter category to list products");
				$('.top_error').show();
			} else {
				flag = true;
