...
<div class="jet-product-index content-section ced-manageproduct">
    <div class="form new-section">
        <?= Html::beginForm(['tophatterrepricing/view'], 'post', ['id' => 'jet_bulk_product']);?>
        <div class="jet-pages-heading tophatter-title">
            ...
            <div class="product-upload-menu">
                ...
                <?= Html::Button('Sync With tophatter', ['class' => 'btn btn-primary', 'onclick' => 'syncBuybox()', 'title' => 'Perform Repricing for the Selected Products.']); ?>
            </div>
            <div class="clear"></div>
        </div>
        ...
        <div class="jet_notice" style="background-color: #FCF8E3;">
            ...
            </span>Don't see all of your products? Just click <a href="<?= yii\helpers\Url::toRoute('categorymap/index'); ?>">here</a> to map all shopify product type(s) with Tophatter category.
        ...
        <?= GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'label' => 'Min Price',
                    'value' => function ($data) {
                        if(count($data['tophatter_product_repricing']))
                        {
                            if(count($data['tophatter_product_repricing']) == 1)
                            {
                                return $data['tophatter_product_repricing'][0]['min_price'];
                            }
                            else
                            {
                                $enabled = 0; $disabled = 0;
                                foreach ($data['tophatter_product_repricing'] as $value) {
                ...
                [
                    ...
                    'value' => function ($data) {
                        if(count($data['tophatter_product_repricing']))
                        {
                            if(count($data['tophatter_product_repricing']) == 1)
                            {
                                return $data['tophatter_product_repricing'][0]['max_price'];
                            }
                            else
                            {
                                $enabled = 0; $disabled = 0;
                ...
                [
                    ...
                    'value' => function ($data) {
                        if(count($data['tophatter_product_repricing']))
                        {
                            if(count($data['tophatter_product_repricing']) == 1)
                            {
                                if($data['tophatter_product_repricing'][0]['buybox'])
                                    return 'WIN';
                                else
                                    return 'LOSE';
                            }
                ...
                [
                    ...
                    'value' => function ($data) {
                        if(count($data['tophatter_product_repricing']))
                        {
                            if(count($data['tophatter_product_repricing']) == 1)
                            {
                                if($data['tophatter_product_repricing'][0]['repricing_status'])
                                    return 'ENABLED';
                                else
                                    return 'DISABLED';
                            }
...
<script type="text/javascript">
    ...
    function syncBuybox()
    {
        window.location.replace("<?= Data::getUrl('tophatterrepricing/syncbuybox') ?>");
    }
