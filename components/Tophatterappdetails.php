...
use frontend\modules\tophatter\models\TophatterExtensionDetail as Detail;

class Tophatterappdetails extends Component
{	
	public static function isValidateapp($merchant_id)
	{
        try
        {
	        ...
	        $model="";
	        $queryObj="";
	        $query = "Select merchant_id,expire_date,status FROM `tophatter_extension_detail` WHERE merchant_id='".$merchant_id."'";
			...
			if($model)
	        {
	        	...
	        	if(time()>$expdate)
	        	{
	        		...
	        		{
			        	$sql="UPDATE `tophatter_extension_detail` SET status='".Detail::STATUS_LICENSE_EXPIRED."' where merchant_id='".$merchant_id."'";
			        	$result = Data::sqlRecords($sql, null, 'update');
	        		}
	        		elseif($model['status']==Detail::STATUS_NOT_PURCHASED)
	        		{
	        			$sql="UPDATE `tophatter_extension_detail` SET status='".Detail::STATUS_TRIAL_EXPIRED."' where merchant_id='".$merchant_id."'";
	        			$result = Data::sqlRecords($sql, null, 'update');
	        			return "not purchase";
	        		}
	        		return "expire";
	...
	public static function appstatus($shop,$connection=null){
		...
		$model="";
		$queryObj="";
		$query = "SELECT `status` FROM `tophatter_shop_details` WHERE `shop_url`='".$shop."'";
	...
	{
		$cron_array = array();
		$query="SELECT shop.merchant_id, consumer_id, secret_key, consumer_channel_type_id, shop_url, token, email FROM `tophatter_configuration` config INNER JOIN `tophatter_shop_details` shop ON (shop.merchant_id = config.merchant_id)";
	...
	public static function isAppConfigured($merchant_id)
	{
		...
		$query="SELECT `consumer_id` FROM `tophatter_configuration` WHERE `merchant_id`=$merchant_id";
		$model = Data::sqlRecords($query, 'one');
		if($model)
			return true;
		else
	...
	public static function validateApiCredentials($consumer_id, $secret_key )
	{
		...
		if(!isset($session['tophatter_configured'])) {

			$tophatterAPi = new Tophatterapi($consumer_id, $secret_key);
			$itemsResult = $tophatterAPi->getItems();
			...
			$session->set('tophatter_configured', true);
			return true;
		} else {
			return true;
		}
	...
	public static function authoriseAppDetails($merchant_id, $shop)
	{
		...
		if(!isset($session['tophatter_installed'])) {
			if(self::appstatus($shop))
				$session->set('tophatter_installed', true);
			else {
				$msg = 'Please install app to continue tophatter integration for your shop store';
				$return['status'] = false;
				$return['message'] = $msg;
			}
		}
		
		if(!isset($session['tophatter_validateapp'])) {
			...
			elseif($isValidate == 'not purchase') {
				...
				$session->set('tophatter_validateapp', true);
		}
