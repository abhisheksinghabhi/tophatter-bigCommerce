...
class TophatterPromoStatus extends component
{
	...
	public static function getPromoStatus($productIds)
	{
		try{
			...
			$query = "";
	        $query = $connection->createCommand("SELECT `sku`,`option_id`,`product_id` FROM `tophatter_promotional_price` WHERE `product_id` IN(".$productIds.") GROUP BY `sku`");
	        ...
	        foreach ($promos as $promo) {
	        	...
	        	$count = 0;
	        	$error = 0;
	        	if(Tophatterapi::is_json($response)){
		        ...
		        if( count($responseArray) > 0 ){
		            	...
		            	else
		            	{
		            		...
		            		if(isset($pricingList['_attribute']))
		            		{
		            			...
		            			$tophatter_status = $status;
		            			$tophatter_promo_id = trim($attributes['promoId']);
			            		...
			            		{
			            			$query = "INSERT INTO `tophatter_promotional_price`(`merchant_id`, `product_id`, `option_id`, `sku`, `effective_date`, `expiration_date`, `current_price_type`, `original_price`, `special_price`, `tophatter_status`, `tophatter_promo_id`) VALUES ('{$merchant_id}', '{$product_id}', '{$option_id}', '{$sku}', '{$effectiveDate}', '{$expirationDate}','{$currentPriceType}','{$original_price}','{$special_price}','{$tophatter_status}','{$tophatter_promo_id}')";
			            		}
			            		else
			            		{
				            		/*$query = "UPDATE `tophatter_promotional_price` SET `tophatter_promo_id`= '".trim($attributes['promoId'])."', `tophatter_status` = '".trim($status)."' WHERE `sku`='".$promo ['sku']."' AND `product_id`='".$promo ['product_id']."' AND `option_id`='".$promo ['option_id']."' AND `effective_date`='".trim($attributes['effectiveDate'])."' AND `expiration_date`= '".trim($attributes['expirationDate'])."'";*/
				            		$uploaded_ids[] = $exist['id'];
				            		$query = "UPDATE `tophatter_promotional_price` SET `tophatter_promo_id`= '".trim($attributes['promoId'])."', `tophatter_status` = '".trim($status)."' WHERE `id`='{$exist['id']}'";
			            		}
	        ...
	        if(count($uploaded_ids)) {
	        	$query = "UPDATE `tophatter_promotional_price` SET `tophatter_promo_id`= '', `tophatter_status` = '".self::PROMOTIONAL_PRICE_STATUS_PENDING."', `to_delete`=0 WHERE `merchant_id`=".MERCHANT_ID." AND `id` NOT IN (".implode(',', $uploaded_ids).")";
	...
	{
		$query = "DELETE FROM `tophatter_promotional_price` WHERE `sku`='{$productSku}' AND `merchant_id`='{$merchant_id}' AND `to_delete`=1 AND `tophatter_status`='{self::PROMOTIONAL_PRICE_STATUS_PROCESSING}'";
	...
	{
		$query = "UPDATE `tophatter_promotional_price` SET `tophatter_status`='{self::PROMOTIONAL_PRICE_STATUS_PENDING}' WHERE `sku`='{$productSku}' AND `merchant_id`='{$merchant_id}' AND `tophatter_status`='{self::PROMOTIONAL_PRICE_STATUS_PROCESSING}'";
	...
	{
		$query = "SELECT COUNT(*) as `promo` FROM `tophatter_promotional_price` WHERE `sku` LIKE '{$productSku}' AND `merchant_id`='{$merchant_id}'";
