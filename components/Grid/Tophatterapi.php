...
//        return $response;
    ...
    public function createProductOnTophatter($ids,$tophatterHelper,$merchant_id,$connection)
    {
                ...
                else
                {
                    ...
                    if($productArray['type']=="simple")
                    {
                        ...
                        if($productArray['tophatter_attributes']!='')
                            ...
                            if($productArray['category'] != '')
                            {
                                if($requiredCategoryAttributes) 
                                {
                                    ...
                                    $tophatter_attributes = json_encode($requiredWalAttrValues);
                                }
                            }
                        }
                        ...
                        if($requiredCategoryAttributes && count($requiredCategoryAttributes['attributes']))
                        {
                            ...
                            $Catdata = self::getCategoryArray($productArray['sku'],null,$productArray['category'],$tophatter_attributes,null,$brand,$productArray['type'],$connection,$productArray['common_attributes'],$variantGroupId);
                    ...
                    else
                    {
                        ...
                        foreach($productVarArray as $value)
                        {
                            ...
                            if($productArray['tophatter_attributes']=='')
                            {
                                $mappedData = AttributeMap::getMappedTophatterAttributes($productArray['product_type'],$value['option_id'],$merchant_id);
                           ...
                           // $requiredCategoryAttributes = AttributeMap::getTophatterCategoryAttributes($productArray['category']);
                            ...
                            if(count($requiredCategoryAttributes['attributes']))
                            {
                                ...
                                $Catdata = self::getCategoryArray($productArray['sku'], $isParent, $productArray['category'], $productArray['tophatter_attributes'], $value['tophatter_option_attributes'], $brand, $productArray['type'], $connection, $productArray['common_attributes'],$productArray['product_id']);
                                if(count($requiredCategoryAttributes['required_attrs']) && (!is_array($Catdata) || (is_array($Catdata) && (!isset($Catdata['category_id']) || !isset($Catdata['attributes']))))) {
                                    $error[$productArray['sku']] = "Please Fill the Required Attributes for Product having sku :'".$productArray['sku']."'";
                                    continue;
                                }
     ...
     public static function prepareProductData($merchant_id, $product, $imageArr, $tax_code, $Catdata, $requiredCategoryAttributes)
    {
        ...
        $validateData = TophatterProductValidate::checkProductBeforeUpload($productData, $product, $requiredCategoryAttributes);
        // by shivam
        $validate = TophatterProductValidate::validateProductXml($productData);
    ...
    public static function validateProduct($product,$connection)
    {
        ...
        if($isexistAttr && !$product['tophatter_attributes']){
            $errorArr[]="Missing tophatter attributes";
        }*/
        //check if tophatter attributes exist
        ...
        if($product['type']=="simple")
        {
            ...
            $tophatterHelper = new Tophatterapi(API_USER,API_PASSWORD,CONSUMER_CHANNEL_TYPE_ID);
            //$validatedPro[$product['sku']] = $tophatterHelper->getItem($product['sku'], 'publishedStatus');
            if($product['status']==TophatterProduct::PRODUCT_STATUS_NOT_UPLOADED)
        ...
        else
        {
            ...
            //check if tophatter attributes not available for category
          /*  if(!$isexistAttr)
            {
                if(($price<=0 || ($price && !is_numeric($price))) || trim($price)==""){
                    $errorArr[]="Missing/invalid price";
           ...
           // else
            //{
            	//print_r($product);die;
                $productVarArray=[];
                $query='select wal.option_id,option_sku,option_image,option_qty,option_price,option_unique_id,wal.status from `jet_product_variants` jet INNER JOIN `tophatter_product_variants` wal ON jet.product_id=wal.product_id where jet.product_id="'.$product['product_id'].'"';
                $productVarArray = Data::sqlRecords($query,"all","select");
    ...
    /**
     * check required category attributes On tophatter
    ...
    public static function checkAttributes($product, $connection=array(),&$isattrexist)
    {
        $category_id = $product['category'];
        $catCollection = [];
        $query = 'select attributes from `tophatter_category` where category_id="'.$category_id.'" LIMIT 1';
        $catCollection = Data::sqlRecords($query, "one", "select");
        if(isset($catCollection['attributes']) && $catCollection['attributes'])
        {
            $isattrexist = true;
            if($product['tophatter_attributes'] != '') {
                return true;
            } else {
     ...
     /**
     * get category attributes and parent id On tophatter
     * @param string|[] $category_id
     * @return bool
     */
    public static function getCategoryArray($sku=NULL,$isParent=NULL,$category_id=NULL,$mappedattributes=NULL,$mappedVarAttr=NULL,$brand=NULL,$type=NULL,$connection,$commonAttributeValues='',$variantGroupId)
    {
        ...
        //check if selected `category_id` exist on tophatter or not
        $catCollection = [];
        $query = "SELECT `parent_id` FROM `tophatter_category` WHERE `category_id`='".$category_id."' LIMIT 0,1";
        $catCollection = Data::sqlRecords($query,"one","select");
        if($catCollection)
        {
            $attrList = [];
            ...
            //get tophatter required attributes
...
//print_r($additionalAttrs);
                if(!is_null($catCollection['parent_id']) && $catCollection['parent_id'] != '0')
                {
                    if(is_array(TophatterCategory::getCategoryOrder($catCollection['parent_id'])) && count(TophatterCategory::getCategoryOrder($catCollection['parent_id']))>0)
                    {
                        $parentAttrList = $attrList;
                        foreach(TophatterCategory::getCategoryOrder($catCollection['parent_id']) as $value)
                        {
                            $attributeArray=[];
                ...
                if(is_array(TophatterCategory::getCategoryOrder($category_id)) && count(TophatterCategory::getCategoryOrder($category_id))>0)
                {
                    $AttrList = $attrList;
                    
                    foreach (TophatterCategory::getCategoryOrder($category_id) as $value)
                    {
                    	//print_r($value);die;
                        $attributeArray=[];
                        $attributeArray = explode("/", $value);
     ...
     * @return bool
     */
    public function updateInventoryOnTophatter($product,$datafrom)
    {
