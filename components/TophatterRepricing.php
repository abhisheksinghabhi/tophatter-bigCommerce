...
class TophatterRepricing extends Tophatterapi
{
	 ...
	 *
	 * @param int|array $tophatterItemId 
	 * @return array 
	 */
	public static function getBestMarketplacePrice($tophatterItemId,$upc=false)
	{
		$apiKey = "q67dm5pkwm36rd366ggzzm5e";
		if(is_numeric($tophatterItemId) && !$upc)
		{
			$url = "http://api.tophatterlabs.com/v1/items/$tophatterItemId?format=json&apiKey=$apiKey";
		}
		elseif(is_array($tophatterItemId) && !$upc)
		{
			//supports upto 20 items in one call
			$tophatterItemIds = implode(',', $tophatterItemId);
			$url = "http://api.tophatterlabs.com/v1/items?format=json&ids=$tophatterItemIds&apiKey=$apiKey";
		}
		elseif($upc)
		{
			$url = "http://api.tophatterlabs.com/v1/items?format=json&apiKey=$apiKey&upc=$tophatterItemId";
		}
		else
		{
			return false;
		...
		{
			return $result = Tophatterapi::xmlToArray($output);
		}
		else
		{
	...
	public static function getSavedRepricingData($product_id)
	{
		try
		{
			$query = 'SELECT * FROM `tophatter_product_repricing` WHERE product_id="'.$product_id.'"';
	...
	public static function getProductData($product_id)
	{
		try
		{
			//$query = 'SELECT product_id,title,sku,type,wal.product_type,wal.status,description,image,qty,price,weight,vendor,upc,tophatter_attributes,category,tax_code,short_description,self_description,common_attributes,attr_ids,sku_override,product_id_override FROM `tophatter_product` wal INNER JOIN `jet_product` jet ON jet.id=wal.product_id WHERE wal.product_id="'.$product_id.'" LIMIT 1';
			$query = 'SELECT product_id,title,sku,type,wal.product_type,wal.status,description,image,qty,price,weight,upc,tophatter_attributes,category,tax_code,short_description,self_description,common_attributes,sku_override,product_id_override FROM (SELECT * FROM `tophatter_product` WHERE `product_id` ="'.$product_id.'") as wal INNER JOIN (SELECT * FROM `jet_product` WHERE `bigproduct_id` ="'.$product_id.'") as jet ON jet.bigproduct_id=wal.product_id WHERE wal.product_id="'.$product_id.'" LIMIT 1';
	...
	public static function getProductVariants($product_id)
	{
		try
		{
			//$query = 'SELECT jet.option_id,option_title,option_sku,wal.status,wal.tophatter_option_attributes,option_image,option_qty,option_price,option_weight,option_unique_id FROM `tophatter_product_variants` wal INNER JOIN `jet_product_variants` jet ON jet.option_id=wal.option_id WHERE wal.product_id="'.$product_id.'"';
			$query = 'SELECT jet.option_id,option_title,option_sku,wal.status,wal.tophatter_option_attributes,option_image,option_qty,option_price,option_weight,option_unique_id FROM (SELECT * FROM `tophatter_product_variants` WHERE `product_id` ="'.$product_id.'") as wal INNER JOIN (SELECT * FROM `jet_product_variants` WHERE `product_id` ="'.$product_id.'") as jet ON jet.option_id=wal.option_id WHERE wal.product_id="'.$product_id.'"';
	...
	public static function getProductIdsBySku($sku, $merchant_id)
	{
		try
		{
			$main_table = "SELECT `product_id`, `variant_id` FROM `tophatter_product` `wp` INNER JOIN (SELECT * FROM `jet_product` WHERE `sku`='{$sku}' AND `merchant_id`={$merchant_id}) as `jp` ON `jp`.`bigproduct_id`=`wp`.`product_id`";


			$variant_table = "SELECT `wpv`.`product_id`, `wpv`.`option_id` as `variant_id` FROM `tophatter_product_variants` `wpv` INNER JOIN (SELECT * FROM `jet_product_variants` WHERE `option_sku`='{$sku}' AND `merchant_id`={$merchant_id}) as `jpv` ON `wpv`.`option_id`=`jpv`.`option_id`";
	...
	{
		$query = "SELECT `id` FROM `tophatter_product_repricing` WHERE `merchant_id` = {$merchant_id} AND `sku` LIKE '{$sku}' LIMIT 0,1";
		$result = Data::sqlRecords($query, "one", "select");
	...
	/**
	 * Get Report of Products on tophatter 
	 * you will get to download .zip file which has the csv file of product report.
	 *
	 * @return void 
	 */
	public function fetchTophatterProductReport()
	{
		$url = Tophatterapi::GET_REPORTS_SUB_URL."?type=item";
	...
	/**
	 * Get Report of Products Buybox on tophatter 
	 * you will get to download .zip file which has the csv file of products buybox.
	 *
	 * @param $filePath
	 * @return void|bool
	 */
	public function fetchTophatterBuyboxReport($filePath=null, $downloadable=false)
	{
		$url = Tophatterapi::GET_REPORTS_SUB_URL."?type=buybox";
	...
	/**
	 * Get Product Price to be Uploaded on tophatter
	 *
	...
	public static function getProductPrice($orignal_price,$type,$id,$merchant_id)
	{
		...
		if($type == 'simple')
		{
			...
			$query = "SELECT * FROM `tophatter_product_repricing` WHERE `product_id` = '{$id}' AND `repricing_status` = '1' LIMIT 0,1";
			$result = Data::sqlRecords($query,"one","select");
			if($result) {
		...
		elseif($type == 'variants')
		{
			...
			$query = "SELECT * FROM `tophatter_product_repricing` WHERE `option_id` = '{$id}' LIMIT 0,1";
			$result = Data::sqlRecords($query,"one","select");
			if($result) {
				$minPrice = floatval($result['min_price']);
				$maxPrice = floatval($result['max_price']);
	...
	public static function downloadBuyBoxReport($merchant_id, $filePath=null)
	{
		...
		$query = "SELECT * FROM `tophatter_buybox_report` WHERE NOW() > ADDTIME(`download_time`, '".$interval_time."') AND `merchant_id`={$merchant_id} LIMIT 0,1";
		$result = Data::sqlRecords($query,'one');

		$isExist = Data::sqlRecords("SELECT * FROM `tophatter_buybox_report` WHERE `merchant_id`={$merchant_id} LIMIT 0,1",'one');
		...
		if($result || !$isExist)
		{
			...
			if($this->fetchTophatterBuyboxReport($filePath)) 
			{
				$time = time();
				if($isExist)
				{
					$query = "UPDATE `tophatter_buybox_report` SET `timestamp` = '{$time}' WHERE `id` = {$result['id']}";
					Data::sqlRecords($query,null,'update');
				}
				else
				{
					$query = "INSERT INTO `tophatter_buybox_report` (`merchant_id`, `timestamp`) VALUES ({$merchant_id},'{$time}')";
	...
	public static function isRepricingEnabled($attribute,$merchant_id=null)
	{
		...
		if($where != '') 
		{
			$query = 'SELECT `id` FROM `tophatter_product_repricing` WHERE '.$where.' AND `repricing_status`=1 AND `merchant_id`="'.$merchant_id.'" LIMIT 0,1';
			$result = Data::sqlRecords($query, 'one');
			if(!$result) {
				return false;
			} else {
